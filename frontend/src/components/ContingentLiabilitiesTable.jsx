function ContingentLiabilitiesTable({ data }) {
  if (!data || !data.data || data.data.length === 0) {
    return (
      <div style={{ padding: "20px", textAlign: "center", color: "#666" }}>
        No Contingent Liabilities and Commitments data available
      </div>
    );
  }
  ...
  const getRowStyle = (row) => {
    const baseStyle = {
      backgroundColor: row.isTotal ? "#e3f2fd" : "#fff",
...
export default ContingentLiabilitiesTable;