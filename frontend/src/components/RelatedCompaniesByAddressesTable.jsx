// FILE: src/components/RelatedCompaniesByAddressesTable.jsx

import { useState } from 'react';
import { formatCurrency } from '../services/extractRelatedCompanies';

function RelatedCompaniesByAddressesTable({ data }) {
  const [expandedCompany, setExpandedCompany] = useState(null);

  if (!data || data.length === 0) {
    return (
      <div style={{ padding: '20px', marginTop: '20px' }}>
        <h2>Related Companies By Address</h2>
        <p style={{ color: '#666' }}>There are no related companies by address.</p>
      </div>
    );
  }
  ...
  return (
    <div style={{ marginTop: '30px' }}>
      <h2 style={{ marginBottom: '10px' }}>Related Companies By Address</h2>
      <p style={{ color: '#666', marginBottom: '20px' }}>
        Companies sharing common addresses with the main entity
      </p>
      ...
      {expandedCompany !== null && (
        <div style={styles.detailCard}>
          ...
          <table style={styles.detailTable}>
            <tbody>
              <tr>
                <td style={styles.detailLabel}>Common Addresses</td>
                <td style={styles.detailValue} colSpan="3">
                  {data[expandedCompany].addresses.join(', ')}
                </td>
              </tr>
              <tr>
...
export default RelatedCompaniesByAddressesTable;